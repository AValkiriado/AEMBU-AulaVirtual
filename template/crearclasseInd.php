...
if(isset($_POST['submit'])&&!empty($_POST['submit'])){
    ...
    if (mysqli_query($conectar, $sql)) {
        ...
        while($cls=mysqli_fetch_assoc($classe)){
            $idClasse = $cls["id"];
            $sql2 = "insert into Usuari_has_Classe (Usuari_username, Classe_id) Values ('$alumne',$idClasse),('$profe',$idClasse)";
...
<main>
<br><br>
<h1 class="main-title">Crear Classe Individual</h1>
<div class="form1-group">
    ...
    <form method="post">
            ...
            <?php
                $Alumnes = mysqli_query($conectar,"select u.nom,u.cognoms,u.username from Usuari u inner join Tipus t on t.id = u.tipus_id where u.Tipus_id =1;");
            ...
            <?php
                $Alumnes = mysqli_query($conectar,"select u.nom,u.cognoms,u.username from Usuari u inner join Tipus t on t.id = u.tipus_id where u.Tipus_id =2;");
                while($alumne=mysqli_fetch_assoc($Alumnes)){
                    $UsrUsrname=$alumne['username'];
                    $UsrNom=$alumne['nom'];
                    $UsrCognom=$alumne['cognoms'];
